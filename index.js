...
function Validator(formSelector) {
    ...
    var validatorRules = {
        required(value) {
            ...
            return value ? '' : `Enter the valid Name`;
        },
        email(value) {
            var regex = /^\w+([\.-]?\w+)*@\w+([\.-]?\w+)*(\.\w{2,3})+$/;
            return regex.test(value) ? '' : `Enter the valid email`;
        },
        min(min) {
            return (value) => {
                return value.length >= min ? '' : `Enter at least ${min} chacter!`;
            }
        },
        max(max) {
            return (value) => {
                return value.length <= max ? '' : `Enter up to ${max} charcters`;
            }
        },
        confirm(selector) {
            return (value) => {
                return value == document.querySelector(selector).value ? '' : `Password does not matchj`;
            }
        },
        checkone(name) {
            return () => {
                var checked = formElement.querySelector(`[name="${name}"]:checked`);
                return checked ? '' : `Please select at leats one`;
            }
        }
